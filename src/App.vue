  ...
  <v-app>
    ...
    <v-footer class="d-flex flex-column" dark>
      <div class="d-flex w-100 align-center px-4">
        ...
        <v-btn icon small class="mx-4"><v-icon>mdi-instagram</v-icon></v-btn>
        <v-btn icon small class="mx-4"><v-icon>mdi-twitter</v-icon></v-btn>
        <v-btn icon small class="mx-4"><v-icon>mdi-facebook</v-icon></v-btn>
        <v-btn icon small class="mx-4"
          ><v-icon>mdi-email-outline</v-icon></v-btn
        >
      </div>

      <div class="px-4 py-2 bg-black text-center w-100">
        ©{{ new Date().getFullYear() }} — <strong>Lanier Luxury Rentals</strong>
      </div>
    </v-footer>
  </v-app>
</template>
